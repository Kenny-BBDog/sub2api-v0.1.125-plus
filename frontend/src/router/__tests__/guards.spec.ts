...
function simulateGuard(
  ...
  const isManager = authState.isManager === true
  const isAdminOrManager = authState.isAdmin || isManager
  ...
  if (!requiresAuth) {
    if (
      ...
      if (authState.backendModeEnabled && !isAdminOrManager) {
        return null
      }
      return isAdminOrManager ? '/admin/dashboard' : '/dashboard'
    }
  ...
  // 需要管理员但不是管理员
  const allowsManager = toMeta.allowsManager === true
  if (requiresAdmin && !(authState.isAdmin || (allowsManager && isManager))) {
    return '/dashboard'
  }
  ...
  if (authState.backendModeEnabled) {
    if (authState.isAuthenticated && isAdminOrManager) {
      return null
    }
    const allowed = ['/login', '/key-usage', '/setup', '/payment/result']
    const callbackPaths = [
