...
function getDefaultBackofficePath(authStore: ReturnType<typeof useAuthStore>): string {
...
router.beforeEach((to, _from, next) => {
  ...
  if (!requiresAuth) {
    ...
    if (authStore.isAuthenticated && (to.path === '/login' || to.path === '/register')) {
      // In backend mode, regular users should NOT be redirected away from login
      // (they are blocked from all protected routes, so redirecting would cause a loop)
      if (appStore.backendModeEnabled && !authStore.isAdminOrManager) {
        next()
        return
      }
      next(getDefaultBackofficePath(authStore))
      return
    }
  ...
  const allowsManager = to.meta.allowsManager === true
  if (requiresAdmin && !(authStore.isAdmin || (allowsManager && authStore.isManager))) {
  ...
  if (to.meta.requiresRiskControl) {
    ...
    if (!riskControlEnabled) {
      next(authStore.isAdmin ? '/admin/settings' : getDefaultBackofficePath(authStore))
      return
    }
  }
  ...
  // Backend mode: backoffice roles get full access, regular users blocked
  if (appStore.backendModeEnabled) {
    if (authStore.isAuthenticated && authStore.isAdminOrManager) {
      next()
      return
    }
    const isAllowed = isBackendModePublicRouteAllowed(to.path, authStore.hasPendingAuthSession)
