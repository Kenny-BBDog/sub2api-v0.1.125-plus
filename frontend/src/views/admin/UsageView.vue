...
import { useAppStore, useAuthStore } from '@/stores'; import { adminAPI } from '@/api/admin'; import { adminUsageAPI } from '@/api/admin/usage'
...
const ADMIN_USAGE_TIMEZONE = 'Asia/Shanghai'
...
const loadStats = async () => {
  ...
  try {
    const requestType = filters.value.request_type
    const legacyStream = requestType ? requestTypeToLegacyStream(requestType) : filters.value.stream
    const s = await adminAPI.usage.getStats({ ...filters.value, timezone: ADMIN_USAGE_TIMEZONE, stream: legacyStream === null ? undefined : legacyStream })
