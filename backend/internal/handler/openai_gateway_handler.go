...
func (h *OpenAIGatewayHandler) validateFunctionCallOutputRequest(c *gin.Context, body []byte, reqLog *zap.Logger) bool {
	...
	h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "function_call_output requires item_reference ids matching each call_id on HTTP requests")
	return false
}
...
func (h *OpenAIGatewayHandler) openAIWSIngressTurnSlotWaitTimeout() time.Duration {
	if h != nil && h.cfg != nil && h.cfg.Gateway.OpenAIWS.IngressTurnSlotWaitTimeoutSeconds > 0 {
		return time.Duration(h.cfg.Gateway.OpenAIWS.IngressTurnSlotWaitTimeoutSeconds) * time.Second
	}
	return 30 * time.Second
}

func (h *OpenAIGatewayHandler) openAIWSIngressTurnSlotMaxWaiting() int {
	if h != nil && h.cfg != nil && h.cfg.Gateway.OpenAIWS.IngressTurnSlotMaxWaiting > 0 {
		return h.cfg.Gateway.OpenAIWS.IngressTurnSlotMaxWaiting
	}
	return 3
}

func (h *OpenAIGatewayHandler) openAIWSIngressWaitPingInterval() time.Duration {
	if h != nil && h.cfg != nil && h.cfg.Gateway.OpenAIWS.IngressWaitPingIntervalSeconds > 0 {
		return time.Duration(h.cfg.Gateway.OpenAIWS.IngressWaitPingIntervalSeconds) * time.Second
	}
	return 10 * time.Second
}

func (h *OpenAIGatewayHandler) acquireOpenAIWSUserSlot(
	c *gin.Context,
	wsConn *coderws.Conn,
	userID int64,
	userConcurrency int,
	reqModel string,
	terminalFailureWritten *bool,
	failAndClose func(coderws.StatusCode, string, string),
	reqLog *zap.Logger,
) (func(), bool) {
	ctx := c.Request.Context()
	userReleaseFunc, userAcquired, err := h.concurrencyHelper.TryAcquireUserSlot(ctx, userID, userConcurrency)
	if err != nil {
		...
		failAndClose(coderws.StatusInternalError, "internal_error", "failed to acquire user concurrency slot")
		return nil, false
	}
	if userAcquired {
	...
	canWait, waitErr := h.concurrencyHelper.IncrementWaitCount(ctx, userID, maxWait)
	if waitErr != nil {
		reqLog.Warn("openai.websocket_user_wait_counter_increment_failed", zap.Error(waitErr))
	} else if !canWait {
		reqLog.Info("openai.websocket_user_wait_queue_full", zap.Int("max_wait", maxWait))
		failAndClose(coderws.StatusTryAgainLater, "user_busy", "too many concurrent requests, please retry later")
		return nil, false
	}
	waitCounted := waitErr == nil && canWait
	defer func() {
	...
	releaseFunc, err := h.waitOpenAIWSUserSlot(ctx, wsConn, userID, userConcurrency, h.openAIWSIngressTurnSlotWaitTimeout(), reqLog, "initial")
	if err != nil {
		reqLog.Warn("openai.websocket_user_slot_acquire_failed_after_wait", zap.Error(err))
		var closeErr *service.OpenAIWSClientCloseError
		if errors.As(err, &closeErr) {
			failAndClose(closeErr.StatusCode(), openAIWSCloseFailureCode(closeErr), closeErr.Reason())
		} else {
			failAndClose(coderws.StatusInternalError, "internal_error", "failed to acquire user concurrency slot")
		}
		return nil, false
	...
	_ = reqModel
	_ = terminalFailureWritten
	return releaseFunc, true
}

func (h *OpenAIGatewayHandler) waitOpenAIWSAccountSlot(
	c *gin.Context,
	wsConn *coderws.Conn,
	accountID int64,
	maxConcurrency int,
	...
	ctx := c.Request.Context()
	if timeout <= 0 {
		timeout = h.openAIWSIngressTurnSlotWaitTimeout()
	}
	if maxWaiting <= 0 {
		maxWaiting = h.openAIWSIngressTurnSlotMaxWaiting()
	}
	canWait, waitErr := h.concurrencyHelper.IncrementAccountWaitCount(ctx, accountID, maxWaiting)
	if waitErr != nil {
		reqLog.Warn("openai.websocket_account_wait_counter_increment_failed", zap.Int64("account_id", accountID), zap.Error(waitErr))
	} else if !canWait {
		...
		failAndClose(coderws.StatusTryAgainLater, "account_busy", "account is busy, please retry later")
		return nil, false
	}
	waitCounted := waitErr == nil && canWait
	defer func() {
	...
	releaseFunc, err := h.waitOpenAIWSAccountSlotAcquire(ctx, wsConn, accountID, maxConcurrency, timeout, reqLog, phase)
	if err != nil {
		reqLog.Warn("openai.websocket_account_slot_acquire_failed_after_wait", zap.Int64("account_id", accountID), zap.Error(err), zap.String("phase", phase))
		var closeErr *service.OpenAIWSClientCloseError
		if errors.As(err, &closeErr) {
			failAndClose(closeErr.StatusCode(), openAIWSCloseFailureCode(closeErr), closeErr.Reason())
		} else {
			failAndClose(coderws.StatusInternalError, "internal_error", "failed to acquire account concurrency slot")
		}
		return nil, false
	}
	if waitCounted {
...
func (h *OpenAIGatewayHandler) acquireOpenAIWSUserSlotForTurn(
	ctx context.Context,
	wsConn *coderws.Conn,
	userID int64,
	userConcurrency int,
	...
	userReleaseFunc, userAcquired, err := h.concurrencyHelper.TryAcquireUserSlot(ctx, userID, userConcurrency)
	if err != nil {
		return nil, false, service.NewOpenAIWSClientCloseError(coderws.StatusInternalError, "failed to acquire user concurrency slot", err)
	}
	if userAcquired {
		return userReleaseFunc, true, nil
	}

	maxWait := service.CalculateMaxWait(userConcurrency)
	canWait, waitErr := h.concurrencyHelper.IncrementWaitCount(ctx, userID, maxWait)
	if waitErr != nil {
		reqLog.Warn("openai.websocket_turn_user_wait_counter_increment_failed", zap.Int("turn", turn), zap.Error(waitErr))
	} else if !canWait {
		return nil, false, service.NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, "too many concurrent requests, please retry later", nil)
	}
	waitCounted := waitErr == nil && canWait
	defer func() {
		if waitCounted {
	...
	releaseFunc, err := h.waitOpenAIWSUserSlot(ctx, wsConn, userID, userConcurrency, h.openAIWSIngressTurnSlotWaitTimeout(), reqLog, fmt.Sprintf("turn_%d", turn))
	if err != nil {
	...
	return releaseFunc, false, nil
}

func (h *OpenAIGatewayHandler) acquireOpenAIWSAccountSlotForTurn(
	ctx context.Context,
	wsConn *coderws.Conn,
	accountID int64,
	maxConcurrency int,
	...
	accountReleaseFunc, accountAcquired, err := h.concurrencyHelper.TryAcquireAccountSlot(ctx, accountID, maxConcurrency)
	if err != nil {
		return nil, false, service.NewOpenAIWSClientCloseError(coderws.StatusInternalError, "failed to acquire account concurrency slot", err)
	}
	if accountAcquired {
	...
	canWait, waitErr := h.concurrencyHelper.IncrementAccountWaitCount(ctx, accountID, maxWaiting)
	if waitErr != nil {
		reqLog.Warn("openai.websocket_turn_account_wait_counter_increment_failed", zap.Int64("account_id", accountID), zap.Int("turn", turn), zap.Error(waitErr))
	} else if !canWait {
		return nil, false, service.NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, "account is busy, please retry later", nil)
	}
	waitCounted := waitErr == nil && canWait
	defer func() {
		if waitCounted {
	...
	releaseFunc, err := h.waitOpenAIWSAccountSlotAcquire(ctx, wsConn, accountID, maxConcurrency, h.openAIWSIngressTurnSlotWaitTimeout(), reqLog, fmt.Sprintf("turn_%d", turn))
	if err != nil {
		return nil, false, err
	}
	if waitCounted {
...
func (h *OpenAIGatewayHandler) waitOpenAIWSUserSlot(
	ctx context.Context,
	wsConn *coderws.Conn,
	userID int64,
	maxConcurrency int,
	...
	return h.waitOpenAIWSSlot(ctx, wsConn, "user", userID, maxConcurrency, timeout, reqLog, phase)
}

func (h *OpenAIGatewayHandler) waitOpenAIWSAccountSlotAcquire(
	ctx context.Context,
	wsConn *coderws.Conn,
	accountID int64,
	maxConcurrency int,
	...
	return h.waitOpenAIWSSlot(ctx, wsConn, "account", accountID, maxConcurrency, timeout, reqLog, phase)
}

func (h *OpenAIGatewayHandler) waitOpenAIWSSlot(
	ctx context.Context,
	wsConn *coderws.Conn,
	slotType string,
	id int64,
	...
	defer cancel()
	pingTicker := time.NewTicker(h.openAIWSIngressWaitPingInterval())
	...
	for {
		...
		case <-waitCtx.Done():
			if slotType == "user" {
				return nil, service.NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, "too many concurrent requests, please retry later", waitCtx.Err())
			}
			return nil, service.NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, "account is busy, please retry later", waitCtx.Err())
		case <-pingTicker.C:
			pingCtx, pingCancel := context.WithTimeout(ctx, 2*time.Second)
			pingErr := wsConn.Ping(pingCtx)
			pingCancel()
			if pingErr != nil {
				return nil, service.NewOpenAIWSClientCloseError(coderws.StatusGoingAway, "client websocket is unavailable while waiting for concurrency slot", pingErr)
			}
			reqLog.Debug("openai.websocket_wait_ping",
				zap.String("slot_type", slotType),
				zap.Int64("id", id),
				zap.String("phase", phase),
		...
		case <-timer.C:
			...
			var err error
			if slotType == "user" {
				releaseFunc, acquired, err = h.concurrencyHelper.TryAcquireUserSlot(waitCtx, id, maxConcurrency)
			} else {
				releaseFunc, acquired, err = h.concurrencyHelper.TryAcquireAccountSlot(waitCtx, id, maxConcurrency)
			}
			if err != nil {
				if slotType == "user" {
			...
			if acquired {
				reqLog.Debug("openai.websocket_wait_slot_acquired",
					zap.String("slot_type", slotType),
					zap.Int64("id", id),
					zap.String("phase", phase),
					zap.Int("attempt", attempt),
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	failAndClose := func(status coderws.StatusCode, code string, reason string) {
		if !terminalFailureWritten {
			writeOpenAIWSTerminalFailure(ctx, wsConn, reqModel, code, reason)
	...
	if decision := h.checkContentModeration(c, reqLog, apiKey, subject, service.ContentModerationProtocolOpenAIResponses, reqModel, firstMessage); decision != nil && decision.Blocked {
		...
		terminalFailureWritten = true
		failAndClose(coderws.StatusPolicyViolation, contentModerationErrorCode(decision), decision.Message)
		return
	}

	if service.IsImageGenerationIntent("/v1/responses", reqModel, firstMessage) && !service.GroupAllowsImageGeneration(apiKey.Group) {
		failAndClose(coderws.StatusPolicyViolation, "image_generation_not_allowed", service.ImageGenerationPermissionMessage())
		return
	}
	...
	userReleaseFunc, acquired := h.acquireOpenAIWSUserSlot(c, wsConn, subject.UserID, subject.Concurrency, reqModel, &terminalFailureWritten, failAndClose, reqLog)
	if !acquired {
		return
	}
	...
	if err := h.billingCacheService.CheckBillingEligibility(ctx, apiKey.User, apiKey, apiKey.Group, subscription); err != nil {
		...
		failAndClose(coderws.StatusPolicyViolation, "billing_check_failed", "billing check failed")
		return
	}
	...
	failedAccountIDs := make(map[int64]struct{})
	wsSwitchCount := 0
	maxWSSwitches := h.maxAccountSwitches
	if maxWSSwitches <= 0 {
		maxWSSwitches = 3
	}
	canSwitchInitialWSAccount := func(accountID int64, reason string) bool {
		if accountID <= 0 || previousResponseID != "" || wsSwitchCount >= maxWSSwitches {
			return false
		}
		failedAccountIDs[accountID] = struct{}{}
		wsSwitchCount++
		h.gatewayService.RecordOpenAIAccountSwitch()
		reqLog.Warn("openai.websocket_account_failover_switching",
			zap.Int64("account_id", accountID),
			zap.String("reason", reason),
			zap.Int("switch_count", wsSwitchCount),
			zap.Int("max_switches", maxWSSwitches),
		)
		return true
	}
	for {
		...
		if err != nil {
			...
			failAndClose(coderws.StatusTryAgainLater, "no_available_account", "no available account")
			return
		}
		if selection == nil || selection.Account == nil {
			failAndClose(coderws.StatusTryAgainLater, "no_available_account", "no available account")
		...
		if !selection.Acquired {
			if selection.WaitPlan == nil {
				if canSwitchInitialWSAccount(account.ID, "account_busy_no_wait_plan") {
					continue
				}
				failAndClose(coderws.StatusTryAgainLater, "account_busy", "account is busy, please retry later")
				return
			...
			if !fastAcquired {
				if canSwitchInitialWSAccount(account.ID, "account_busy_fast_acquire") {
					continue
				}
				waitRelease, waited := h.waitOpenAIWSAccountSlot(
					c,
					wsConn,
					account.ID,
					selection.WaitPlan.MaxConcurrency,
				...
				accountReleaseFunc = waitRelease
			} else {
		...
		if err != nil {
			...
			failAndClose(coderws.StatusInternalError, "token_unavailable", "failed to get access token")
			return
		}
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			BeforeTurn: func(turn int) error {
				...
				releaseTurnSlots()
				userReleaseFunc, userAcquired, err := h.acquireOpenAIWSUserSlotForTurn(ctx, wsConn, subject.UserID, subject.Concurrency, reqLog, turn)
				if err != nil {
					return err
				}
				accountReleaseFunc, accountAcquired, err := h.acquireOpenAIWSAccountSlotForTurn(ctx, wsConn, account.ID, accountMaxConcurrency, reqLog, turn)
				if err != nil {
					if userReleaseFunc != nil {
						userReleaseFunc()
					}
				...
				currentUserRelease = wrapReleaseOnDone(ctx, userReleaseFunc)
				currentAccountRelease = wrapReleaseOnDone(ctx, accountReleaseFunc)
				reqLog.Debug("openai.websocket_turn_slots_acquired",
					zap.Int64("account_id", account.ID),
					zap.Int("turn", turn),
					zap.Bool("user_waited", !userAcquired),
					zap.Bool("account_waited", !accountAcquired),
				)
				return nil
			},
		...
		if err := h.gatewayService.ProxyResponsesWebSocketFromClient(ctx, c, wsConn, account, token, wsFirstMessage, hooks); err != nil {
			...
			if isRetryableOpenAIWSInitialAcquireError(err) {
				if currentAccountRelease != nil {
					currentAccountRelease()
					currentAccountRelease = nil
				}
				if canSwitchInitialWSAccount(account.ID, "upstream_acquire_busy") {
					continue
				}
			}
			var closeErr *service.OpenAIWSClientCloseError
			...
			failAndClose(coderws.StatusInternalError, "upstream_websocket_failed", "upstream websocket proxy failed")
			return
		}
...
func writeOpenAIWSTerminalFailure(ctx context.Context, conn *coderws.Conn, model string, code string, message string) {
	...
	now := time.Now().Unix()
	responseID := fmt.Sprintf("resp_local_%d", time.Now().UnixNano())
	errObj := gin.H{
		"type":    "server_error",
		"code":    code,
		"message": message,
...
func openAIWSCloseFailureCode(closeErr *service.OpenAIWSClientCloseError) string {
	if closeErr == nil {
		return "upstream_websocket_failed"
	}
	reason := strings.ToLower(strings.TrimSpace(closeErr.Reason()))
	switch {
	case strings.Contains(reason, "account is busy"):
		return "account_busy"
	case strings.Contains(reason, "too many concurrent"):
		return "user_busy"
	case strings.Contains(reason, "no available account"):
		return "no_available_account"
	case strings.Contains(reason, "content moderation") || strings.Contains(reason, "blocked"):
		return "content_policy_violation"
	case strings.Contains(reason, "billing"):
		return "billing_check_failed"
	case strings.Contains(reason, "context window"):
		return "context_length_exceeded"
	default:
		if closeErr.StatusCode() == coderws.StatusTryAgainLater {
...
func isRetryableOpenAIWSInitialAcquireError(err error) bool {
	if err == nil {
		return false
	}
	...
	return strings.Contains(msg, "acquire upstream websocket") &&
		(strings.Contains(msg, "upstream websocket is busy") ||
			strings.Contains(msg, "context deadline exceeded") ||
			strings.Contains(msg, "connection is unavailable")) ||
		(strings.Contains(msg, "upstream_rate_limited") ||
			strings.Contains(msg, "upstream rate limit") ||
			strings.Contains(msg, "usage limit"))
}

func sleepOpenAIWSTurnAcquireBackoff(ctx context.Context, deadline time.Time) bool {
	if ctx == nil {
		ctx = context.Background()
	}
