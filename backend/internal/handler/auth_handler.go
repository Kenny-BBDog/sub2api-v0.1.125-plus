...
func (h *AuthHandler) ensureBackendModeAllowsUser(ctx context.Context, user *service.User) error {
	...
	if h == nil || !h.isBackendModeEnabled(ctx) || user.IsAdminOrManager() {
		return nil
	}
	return infraerrors.Forbidden("BACKEND_MODE_ADMIN_ONLY", "Backend mode is active. Only admin or manager login is allowed.")
}
...
func (h *AuthHandler) RefreshToken(c *gin.Context) {
	...
	if h.settingSvc.IsBackendModeEnabled(c.Request.Context()) &&
		result.UserRole != service.RoleAdmin &&
		result.UserRole != service.RoleManager {
		response.Forbidden(c, "Backend mode is active. Only admin or manager login is allowed.")
		return
	}
