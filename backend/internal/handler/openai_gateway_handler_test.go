...
func TestOpenAIResponses_HTTPContinuationPreviousResponseIDIsNotRejectedAtHandlerEdge(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestOpenAIResponsesWebSocket_PreviousResponseIDKindLoggedBeforeAcquireFailure(t *testing.T) {
	...
	require.Equal(t, "internal_error", gjson.GetBytes(payload, "error.code").String())
	require.Contains(t, strings.ToLower(gjson.GetBytes(payload, "error.message").String()), "failed to acquire user concurrency slot")
...
func TestOpenAIResponsesWebSocketFailureHelpers(t *testing.T) {
	require.True(t, isRetryableOpenAIWSInitialAcquireError(errors.New("acquire upstream websocket: openai ws client close: 1013 upstream websocket is busy, please retry later: context deadline exceeded")))
	require.False(t, isRetryableOpenAIWSInitialAcquireError(errors.New("read upstream websocket: unexpected close")))

	require.Equal(t, "account_busy", openAIWSCloseFailureCode(mustOpenAIWSClientCloseError(t, service.NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, "account is busy, please retry later", nil))))
	require.Equal(t, "user_busy", openAIWSCloseFailureCode(mustOpenAIWSClientCloseError(t, service.NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, "too many concurrent requests, please retry later", nil))))
	require.Equal(t, "invalid_request_error", openAIWSCloseFailureCode(mustOpenAIWSClientCloseError(t, service.NewOpenAIWSClientCloseError(coderws.StatusPolicyViolation, "unsupported websocket message type", nil))))
}

func TestOpenAIResponsesWebSocket_WaitsForInitialUserSlotAndCompletes(t *testing.T) {
	gin.SetMode(gin.TestMode)

	var userAcquireCalls atomic.Int32
	var userWaitIncrements atomic.Int32
	var userWaitDecrements atomic.Int32
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload: `{"type":"response.create","model":"gpt-5.4","stream":false}`,
		concurrencyCache: &concurrencyCacheMock{
			acquireUserSlotFn: func(ctx context.Context, userID int64, maxConcurrency int, requestID string) (bool, error) {
				return userAcquireCalls.Add(1) >= 3, nil
			},
			acquireAccountSlotFn: func(ctx context.Context, accountID int64, maxConcurrency int, requestID string) (bool, error) {
				return true, nil
			},
			incrementUserWaitCountFn: func(ctx context.Context, userID int64, maxWait int) (bool, error) {
				userWaitIncrements.Add(1)
				return true, nil
			},
			decrementUserWaitCountFn: func(ctx context.Context, userID int64) error {
				userWaitDecrements.Add(1)
				return nil
			},
		},
		openAIWSMutateConfig: func(cfg *config.Config) {
			cfg.Gateway.OpenAIWS.IngressTurnSlotWaitTimeoutSeconds = 2
			cfg.Gateway.OpenAIWS.IngressWaitPingIntervalSeconds = 1
		},
	})
	require.NotNil(t, got.log)
	require.Equal(t, int32(3), userAcquireCalls.Load())
	require.Equal(t, int32(1), userWaitIncrements.Load())
	require.Equal(t, int32(1), userWaitDecrements.Load())
}

func TestOpenAIResponsesWebSocket_WaitsForTurnAccountSlotAndCompletes(t *testing.T) {
	gin.SetMode(gin.TestMode)

	var accountAcquireCalls atomic.Int32
	var accountWaitIncrements atomic.Int32
	var accountWaitDecrements atomic.Int32

	cfg := &config.Config{}
	cfg.Gateway.OpenAIWS.IngressTurnSlotWaitTimeoutSeconds = 2
	cfg.Gateway.OpenAIWS.IngressTurnSlotMaxWaiting = 2
	cfg.Gateway.OpenAIWS.IngressWaitPingIntervalSeconds = 1
	cache := &concurrencyCacheMock{
		acquireAccountSlotFn: func(ctx context.Context, accountID int64, maxConcurrency int, requestID string) (bool, error) {
			return accountAcquireCalls.Add(1) >= 3, nil
		},
		incrementAccountWaitCountFn: func(ctx context.Context, accountID int64, maxWait int) (bool, error) {
			accountWaitIncrements.Add(1)
			return true, nil
		},
		decrementAccountWaitCountFn: func(ctx context.Context, accountID int64) error {
			accountWaitDecrements.Add(1)
			return nil
		},
	}
	h := &OpenAIGatewayHandler{
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		release, acquiredImmediately, err := h.acquireOpenAIWSAccountSlotForTurn(r.Context(), conn, 9901, 1, zap.NewNop(), 2)
		if release != nil {
			release()
		}
		if acquiredImmediately {
			resultCh <- errors.New("account slot should have waited before acquiring")
			return
		}
		resultCh <- err
	}))
	...
	case <-time.After(3 * time.Second):
		t.Fatal("timed out waiting for account slot acquisition")
	}
	require.Equal(t, int32(3), accountAcquireCalls.Load())
	require.Equal(t, int32(1), accountWaitIncrements.Load())
	require.Equal(t, int32(1), accountWaitDecrements.Load())
}

func mustOpenAIWSClientCloseError(t *testing.T, err error) *service.OpenAIWSClientCloseError {
	t.Helper()
...
type openAIResponsesWSUsageLogCase struct {
	...
	channelMapping       map[string]string
	concurrencyCache     *concurrencyCacheMock
	openAIWSMutateConfig func(*config.Config)
}
...
func runOpenAIResponsesWebSocketUsageLogCase(t *testing.T, tc openAIResponsesWSUsageLogCase) openAIResponsesWSUsageLogResult {
	...
	if tc.openAIWSMutateConfig != nil {
		tc.openAIWSMutateConfig(cfg)
	}
