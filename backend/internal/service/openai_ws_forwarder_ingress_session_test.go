...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_UpstreamErrorTerminatesTurn(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		ID:          115,
		Name:        "openai-ingress-error-terminal",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	msgType, terminalEvent, readErr := clientConn.Read(readCtx)
	cancelRead()
	require.NoError(t, readErr)
	require.Equal(t, coderws.MessageText, msgType)
	require.Equal(t, "response.failed", gjson.GetBytes(terminalEvent, "type").String())
	require.Equal(t, "failed", gjson.GetBytes(terminalEvent, "response.status").String())
	require.Equal(t, "usage_limit_reached", gjson.GetBytes(terminalEvent, "response.error.type").String())
	require.Contains(t, string(terminalEvent), "The usage limit has been reached")
	require.True(t, <-turnWSModeCh, "上游 error 也应作为 WS turn 终止，而不是等 EOF")
	...
	require.True(t, captureConn.closed, "上游 error 后不应复用该 WS 连接")
}
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_StoreDisabledReplayFunctionCallOutputPreflightPingFailKeepsPrevIDOnFreshConn(t *testing.T) {
	gin.SetMode(gin.TestMode)
	prevPreflightPingIdle := openAIWSIngressPreflightPingIdle
	openAIWSIngressPreflightPingIdle = 0
	defer func() {
	...
	account := &Account{
		ID:          242,
		Name:        "openai-ingress-replay-fco-skip-ping-drop",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"input":[{"type":"function_call_output","call_id":"call_replay_1","output":"ok"}]}`)
	firstTurn := readMessage()
	require.Equal(t, "resp_turn_ping_tool_1", gjson.GetBytes(firstTurn, "response.id").String())

	writeMessage(`{"type":"response.create","model":"gpt-5.1","stream":false,"store":false,"previous_response_id":"resp_turn_ping_tool_1","input":[{"type":"input_text","text":"after tool"}]}`)
	secondTurn := readMessage()
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("等待 ingress websocket 工具续链预检失败结束超时")
	}

	require.Equal(t, 2, dialer.DialCount(), "replay input 含 function_call_output 时应换新连接并保留 previous_response_id 重放")
	require.Equal(t, 1, firstConn.WriteCount(), "预检失败后不应继续写入旧连接")
	require.GreaterOrEqual(t, firstConn.PingCount(), 1, "第二轮前应执行 preflight ping")
	secondConn.mu.Lock()
	secondWrites := append([]map[string]any(nil), secondConn.writes...)
	secondConn.mu.Unlock()
	require.Len(t, secondWrites, 1)
	secondWrite := requestToJSONString(secondWrites[0])
	require.Equal(t, "resp_turn_ping_tool_1", gjson.Get(secondWrite, "previous_response_id").String(), "function_call_output 续链换连时必须保留 previous_response_id")
	require.Equal(t, "input_text", gjson.Get(secondWrite, "input.0.type").String(), "换连重放应保留当前轮输入，并依赖 previous_response_id 关联工具上下文")
}
