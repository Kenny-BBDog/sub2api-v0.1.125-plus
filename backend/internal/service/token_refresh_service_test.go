...
func (r *tokenRefreshAccountRepo) SetSchedulable(ctx context.Context, id int64, schedulable bool) error {
...
func TestTokenRefreshService_RefreshWithRetry_NoRefreshTokenDoesNotTempUnschedule(t *testing.T) {
	...
	require.Equal(t, 1, repo.setSchedulableCalls, "non-retryable credential states should be removed from scheduling")
	require.Equal(t, 1, repo.clearTempCalls, "non-retryable credential states should clear temporary retry state")
}
...
func TestIsNonRetryableRefreshError(t *testing.T) {
	tests := []struct {
		...
		{name: "refresh_token_reused", err: errors.New("code=refresh_token_reused message=Your refresh token has already been used"), expected: true},
