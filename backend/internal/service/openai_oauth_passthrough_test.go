...
func TestOpenAIGatewayService_OAuthHTTPInputTextItemsAreWrappedForUpstream(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	originalBody := []byte(`{"model":"gpt-5.5","stream":false,"input":[{"type":"input_text","text":"hi"}]}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", bytes.NewReader(originalBody))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusBadRequest,
		Header:     http.Header{"Content-Type": []string{"application/json"}, "x-request-id": []string{"rid_input_wrap"}},
...
func TestOpenAIGatewayService_OAuthPassthrough_ResponseHeadersSuppressXCodexUsage(t *testing.T) {
	gin.SetMode(gin.TestMode)
