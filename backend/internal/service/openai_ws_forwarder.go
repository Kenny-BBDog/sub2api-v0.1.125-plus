...
func buildOpenAIWSErrorTerminalEvent(responseID, code, errType, errMessage string) []byte {
	errObj := map[string]any{}
	if code = strings.TrimSpace(code); code != "" {
		errObj["code"] = code
	}
	if errType = strings.TrimSpace(errType); errType != "" {
		errObj["type"] = errType
	}
	if errMessage = strings.TrimSpace(errMessage); errMessage != "" {
		errObj["message"] = errMessage
	} else {
		errObj["message"] = "upstream websocket returned an error event"
	}
	resp := map[string]any{
		"status": "failed",
		"error":  errObj,
	...
	if err != nil {
		return []byte(`{"type":"response.failed","response":{"status":"failed","error":{"message":"upstream websocket returned an error event"}},"error":{"message":"upstream websocket returned an error event"}}`)
	}
	return payload
}
...
func openAIWSRawInputSequenceHasFunctionCallOutput(items []json.RawMessage, exists bool) bool {
	if !exists {
		return false
	}
...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, imageBillingModel string, imageSizeTier string) (*OpenAIForwardResult, error) {
		...
		firstEventType := ""
		lastEventType := ""
		terminateSessionAfterTerminal := false
		...
		for {
			...
			if eventType == "error" {
				...
				// Upstream websocket error events are terminal for the current turn even when
				// the upstream does not follow with response.completed. Emit a canonical
				// response.failed so Codex sees a completed terminal state instead of a late EOF.
				if !recoverablePrevNotFound {
					upstreamMessage = buildOpenAIWSErrorTerminalEvent(responseID, errCodeRaw, errTypeRaw, errMsgRaw)
					eventType = "response.failed"
					terminateSessionAfterTerminal = true
					lease.MarkBroken()
					logOpenAIWSModeInfo(
						"ingress_ws_error_terminal_synthesized account_id=%d turn=%d conn_id=%s idx=%d fallback_reason=%s err_code=%s err_type=%s err_message=%s response_id=%s",
						account.ID,
						turn,
						truncateOpenAIWSLogValue(lease.ConnID(), openAIWSIDValueMaxLen),
						eventCount,
			...
			if isTerminalEvent {
				...
				result := &OpenAIForwardResult{
					...
					FirstTokenMs:             firstTokenMs,
					OpenAIWSTerminateSession: terminateSessionAfterTerminal,
				}
				if imageCount > 0 {
	...
	turnStrictFreshConnRetried := false
	...
	recoverIngressPrevResponseNotFound := func(relayErr error, turn int, connID string) bool {
		...
		if gjson.GetBytes(currentPayload, `input.#(type=="function_call_output")`).Exists() ||
			openAIWSRawInputSequenceHasFunctionCallOutput(currentTurnReplayInput, currentTurnReplayInputExists) {
			return false
		}
	...
	for {
		...
		forcePreferredConn := isStrictAffinityTurn(currentPayload) && !turnStrictFreshConnRetried
		if sessionLease == nil {
		...
		if shouldPreflightPing {
			if pingErr := sessionLease.PingWithTimeout(openAIWSConnHealthCheckTO); pingErr != nil {
				...
				if forcePreferredConn {
					...
					if !turnStrictFreshConnRetried && hasFCOutput {
						logOpenAIWSModeInfo(
							"ingress_ws_preflight_ping_recovery account_id=%d turn=%d conn_id=%s action=fresh_conn_retry_keep_previous_response_id previous_response_id=%s has_function_call_output=%v",
							account.ID,
							turn,
							truncateOpenAIWSLogValue(sessionConnID, openAIWSIDValueMaxLen),
							truncateOpenAIWSLogValue(currentPreviousResponseID, openAIWSIDValueMaxLen),
						...
						turnStrictFreshConnRetried = true
						resetSessionLease(true)
						skipBeforeTurn = true
						continue
					}
		...
		if result.OpenAIWSTerminateSession {
			logOpenAIWSModeInfo(
				"ingress_ws_terminal_session_closed account_id=%d turn=%d conn_id=%s response_id=%s",
				account.ID,
				turn,
				truncateOpenAIWSLogValue(connID, openAIWSIDValueMaxLen),
				truncateOpenAIWSLogValue(responseID, openAIWSIDValueMaxLen),
