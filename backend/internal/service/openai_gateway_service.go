...
type OpenAIForwardResult struct {
	...
	ImageCount      int
	ImageSize       string
	// OpenAIWSTerminateSession is used by ingress WS mode after synthesizing a
	// terminal failure event; the client already received a terminal state and
	// should not be kept waiting for another turn.
	OpenAIWSTerminateSession bool
}
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.Type == AccountTypeOAuth && normalizeOpenAIOAuthResponsesInputItems(reqBody) {
		bodyModified = true
		disablePatch()
	}
...
func normalizeOpenAIPassthroughOAuthBody(body []byte, compact bool) ([]byte, bool, error) {
	...
	if normalizeOpenAIOAuthResponsesInputItems(reqBody) {
		next, err := json.Marshal(reqBody)
		if err != nil {
			return body, false, fmt.Errorf("normalize passthrough oauth input items: %w", err)
		}
		normalized = next
		changed = true
	}
