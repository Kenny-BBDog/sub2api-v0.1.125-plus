...
func (s *TokenRefreshService) refreshWithRetry(ctx context.Context, account *Account, refresher TokenRefresher, executor OAuthRefreshExecutor, refreshWindow time.Duration) error {
	...
	for attempt := 1; attempt <= s.cfg.MaxRetries; attempt++ {
		...
		if isNonRetryableRefreshError(err) {
			...
			if setSchedErr := s.accountRepo.SetSchedulable(ctx, account.ID, false); setSchedErr != nil {
				slog.Warn("token_refresh.set_unschedulable_failed",
					"account_id", account.ID,
					"error", setSchedErr,
				)
			}
			if clearTempErr := s.accountRepo.ClearTempUnschedulable(ctx, account.ID); clearTempErr != nil {
...
func isNonRetryableRefreshError(err error) bool {
	...
	nonRetryable := []string{
		...
		"refresh_token_reused", // OpenAI refresh token 已被其它路径消费，继续重试只会刷屏
	}
