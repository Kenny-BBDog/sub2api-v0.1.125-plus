...
type GatewayOpenAIWSConfig struct {
	...
	// IngressTurnSlotWaitTimeoutSeconds: WS ingress 后续 turn 等待并发槽位的最大秒数。
	IngressTurnSlotWaitTimeoutSeconds int `mapstructure:"ingress_turn_slot_wait_timeout_seconds"`
	// IngressTurnSlotMaxWaiting: WS ingress 后续 turn 单账号等待队列上限。
	IngressTurnSlotMaxWaiting int `mapstructure:"ingress_turn_slot_max_waiting"`
	// IngressWaitPingIntervalSeconds: WS ingress 等待槽位时给客户端发送 ping 控制帧的间隔。
	IngressWaitPingIntervalSeconds int `mapstructure:"ingress_wait_ping_interval_seconds"`
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_ws.ingress_turn_slot_wait_timeout_seconds", 30)
	viper.SetDefault("gateway.openai_ws.ingress_turn_slot_max_waiting", 3)
	viper.SetDefault("gateway.openai_ws.ingress_wait_ping_interval_seconds", 10)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.OpenAIWS.IngressTurnSlotWaitTimeoutSeconds < 0 {
		return fmt.Errorf("gateway.openai_ws.ingress_turn_slot_wait_timeout_seconds must be non-negative")
	}
	if c.Gateway.OpenAIWS.IngressTurnSlotMaxWaiting < 0 {
		return fmt.Errorf("gateway.openai_ws.ingress_turn_slot_max_waiting must be non-negative")
	}
	if c.Gateway.OpenAIWS.IngressWaitPingIntervalSeconds < 1 || c.Gateway.OpenAIWS.IngressWaitPingIntervalSeconds > 30 {
		return fmt.Errorf("gateway.openai_ws.ingress_wait_ping_interval_seconds must be between 1-30")
	}
