...
func TestSimpleModeBypassesQuotaCheck(t *testing.T) {
	...
	t.Run("standard_mode_exposes_context_before_subscription_limit_abort", func(t *testing.T) {
		cfg := &config.Config{RunMode: config.RunModeStandard}
		apiKeyService := service.NewAPIKeyService(apiKeyRepo, nil, nil, nil, nil, nil, cfg)
		...
		var gotAPIKey *service.APIKey
		var gotUser AuthSubject
		var gotGroup *service.Group
		router := gin.New()
		router.Use(func(c *gin.Context) {
			c.Next()
			gotAPIKey, _ = GetAPIKeyFromContext(c)
			if v, ok := c.Get(string(ContextKeyUser)); ok {
				gotUser, _ = v.(AuthSubject)
			}
			gotGroup, _ = c.Request.Context().Value(ctxkey.Group).(*service.Group)
		})
		...
		require.Equal(t, http.StatusTooManyRequests, w.Code)
		require.NotNil(t, gotAPIKey)
		require.Equal(t, apiKey.ID, gotAPIKey.ID)
		require.Equal(t, AuthSubject{UserID: user.ID, Concurrency: user.Concurrency}, gotUser)
